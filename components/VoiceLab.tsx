...
export const VoiceLab: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if ('webkitSpeechRecognition' in window || 'SpeechRecognition' in window) {
      ...
      recog.onresult = (event: any) => {
        let final = '';
        // let interim = ''; // Optional: Use for live preview
  ...
  const toggleListening = () => {
    ...
    if (isListening) {
      ...
      try {
        ...
        setSttError("Error iniciando mic.");
      }
    }
  };
  ...
  const speakText = (text: string) => {
    ...
    utterance.onerror = (event: any) => {
      ...
      // We check event.error string to identify the cause.
      ...
      // Optional: Visual feedback if needed, but console is usually enough for this lab
      setIsSpeaking(false);
    }
